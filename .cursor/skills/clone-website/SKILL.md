...
name: clone-website
description: >-
  Clona ou reconstrói sites a partir de URL(s): extração de assets/CSS/conteúdo, specs por secção e QA visual.
  Use quando o pedido for clonar, replicar, copiar pixel-perfect, reverse-engineer de uma página ou site.
---

# Clone Website

You are about to reverse-engineer and rebuild **the target URL(s) the user provides** as pixel-perfect clones.

## Contexto — Recovery Engine Brasil (este monorepo)

Este repositório **não** é o template Next.js original: o front web está em `apps/web` (Vite + React + Tailwind). Ao aplicar fases que mencionam `src/app/`, `next/font`, etc., **adapta** para a stack real (por exemplo `apps/web/src/`, `index.html`, rotas em `main.tsx`) e valida com `npm run build -w @re/web`. Mantém `docs/research/` na raiz do monorepo salvo indicação em contrário.
...
The target is whatever page the user’s URL(s) resolve to. Clone exactly what's visible at that URL. Unless the user specifies otherwise, use these defaults:
...
2. Parse the user’s input as one or more URLs. Normalize and validate each URL; if any are invalid, ask the user to correct them before proceeding. For each valid URL, verify it is accessible via your browser MCP tool.
3. Verify the project builds (neste monorepo: `npm run build -w @re/web` ou `npm run build` na raiz). Se o alvo for outro scaffold (ex.: template Next do repositório original), ajusta o comando ao `package.json` real.
